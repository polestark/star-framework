...
public class Win32GuiByAu3 {
	...
	/**
	 * upload file in win32 gui using autoit compiled exe</BR> you can use it like this:
	 * fileUpload("选择文件", "D:\\a.txt", 5);
	 * 
	...
	/**
	 * upload file in win32 gui using autoit compiled exe</BR> wait for upload dialog appers in 20
	 * seconds</BR> you can use it like this: fileUpload("选择文件", "D:\\a.txt");
	 * 
	 * @param title dialog title
	 * @param fileName file from where to upload
	 * @throws RuntimeException
	...
	/**
	 * download file in win32 gui using autoit compiled exe</BR> you can use it like this:
	 * fileDownload("文件下载", "另存为", "D:\\download.csv", 30);
	 * 
	...
	public void fileDownload(String fstTitle, String sndTitle, String saveAs, int timeout) {
		...
		String cmd = "\"" + fileExec + "\" \"" + fstTitle + "\" \"" + sndTitle + "\" \"" + saveAs
				+ "\" \"" + timeout + "\"";
		try {
			setExecuteTimeout(timeout);
			execute.executeCommands(cmd);
	...
	/**
	 * download file in win32 gui using autoit compiled exe</BR> wait for download dialog appers in
	 * 20 seconds</BR> you can use it like this: fileDownload("文件下载", "另存为", "D:\\download.csv");
	 * 
	 * @param fstTitle the first download dialog title
	 * @param sndTitle the second dialog after click "Save"
	 * @param saveAs file name to be saved as
	...
	/**
	 * click alert dialog in win32 gui using autoit compiled exe</BR> you can use it like this:
	 * clickAlert("提示信息", "确定(Y)", 10);
	 * 
	...
	public void clickAlert(String dialogTitle, String buttonName, int timeout) {
		...
		String cmd = "\"" + fileExec + "\" \"" + dialogTitle + "\" \"" + buttonName + "\" \""
				+ timeout + "\"";
		try {
			setExecuteTimeout(timeout);
			execute.executeCommands(cmd);
	...
	/**
	 * click alert dialog in win32 gui using autoit compiled exe</BR> wait for alert appers in 5
	 * seconds</BR> you can use it like this: clickAlert("提示信息", "确定(Y)");
	 * 
	 * @param dialogTitle dialog title
	 * @param buttonName the button name/text on the dialog to click
	 * @throws RuntimeException
	...
	/**
	 * type text in alert dialog in win32 gui using autoit compiled exe</BR> you can use it like
	 * this: typeAlert("窗口标题", "Edit1", "输入内容", 5);
	 * 
	...
	public void typeAlert(String title, String locator, String text, int timeout) {
		...
		String cmd = "\"" + fileExec + "\" \"" + title + "\" \"" + locator + "\" \"" + text
				+ "\" \"" + timeout + "\"";
		try {
			setExecuteTimeout(timeout);
			execute.executeCommands(cmd);
	...
	/**
	 * type text in alert dialog in win32 gui using autoit compiled exe</BR> wait for alert appers
	 * in 5 seconds</BR> you can use it like this: typeAlert("窗口标题", "Edit1", "输入内容");
	 * 
	 * @param title init dialog title
	 * @param locator the locator on the dialog
	 * @param text text to put into the edit
	...
	/**
	 * close window by name in win32 gui using autoit compiled exe</BR> you can use it like this:
	 * closeWindow("窗口标题", 5);
	 * 
	 * @param title dialog title
	 * @param timeout timeout setting for wait alert appears in second unit
	...
	/**
	 * close window by name in win32 gui using autoit compiled exe</BR> wait for alert appers in 5
	 * seconds</BR> you can use it like this: closeWindow("窗口标题");
	 * 
	 * @param title dialog title
	 * @throws RuntimeException
	 **/
	...
	/**
	 * activate window by name in win32 gui using autoit compiled exe</BR> you can use it like this:
	 * activateWindow("窗口标题", 5);
	 * 
	 * @param title dialog title
	 * @param timeout timeout setting for wait alert appears in second unit
	 * @throws RuntimeException
	...
	/**
	 * get and write browser message to file using autoit compiled exe</BR> you can use it like
	 * this: assertErrors(title, "errorMessage", "li", "D:\\a.txt", 10);
	 * 
	 ...
	 * @throws RuntimeException
	 **/
	public void assertErrors(String title, String upIdName, String eleType, String fileName,
			int timeout) {
