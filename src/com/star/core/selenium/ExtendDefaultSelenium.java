...
public class ExtendDefaultSelenium extends DefaultSelenium implements ExtendSelenium {
	...
	/**
	 * get the current webtable's row count, on condition initialize completed. put this code into
	 * selenium-server-standalone.jar/core/scripts/user-extension.js:
	 * 
	 * Selenium.prototype.getTabRowCount = function(locator) { var table =
	 * this.browserbot.findElement(locator); return table.rows.length.toString(); };
	 * 
	...
	/**
	 * get the current webtable's column count, on condition initialize completed. put this code
	 * into selenium-server-standalone.jar/core/scripts/user-extension.js:
	 * 
	 * Selenium.prototype.getTabColCount = function(locator, rownum) { var table =
	 * this.browserbot.findElement(locator); return table.rows[rownum - 1].cells.length.toString();
	 * };
	 * 
	 * @param locator the locator of the webtable
	...
	public String getTabColCount(String locator, String rowIndex) {
		return this.commandProcessor
				.getString("getTabColCount", new String[] { locator, rowIndex });
	}
}